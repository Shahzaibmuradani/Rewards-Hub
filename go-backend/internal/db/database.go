...
// createTables initializes the database schema required by the rewards APIs.
...
// seedInitialData populates the dummy data used by the mobile rewards examples.
func seedInitialData(db *sql.DB) error {
	...
	ON CONFLICT (id) DO UPDATE SET
		...
		balance_gems = EXCLUDED.balance_gems,
	...
	ON CONFLICT (id) DO UPDATE SET
		...
		estimated_gems_hour = EXCLUDED.estimated_gems_hour,
		total_gem_pool = EXCLUDED.total_gem_pool,
		updated_at = CURRENT_TIMESTAMP;
